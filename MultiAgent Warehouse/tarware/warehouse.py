...
class Warehouse(gym.Env):
    ...
    def visualize_paths(self):
        ...
        ax.set_title("Agent Path Visualization")
        
        ax.set_xlim(0, self.grid_size[1])
        ax.set_ylim(0, self.grid_size[0])
        ax.set_aspect('equal', 'box')

        colors = plt.cm.jet(np.linspace(0, 1, len(self.agents)))
        
        for agent, color in zip(self.agents, colors):
            if agent.path:
                path_coords = np.array(agent.path)
                ax.plot(path_coords[:, 1], path_coords[:, 0], color=color, marker='o', label=f"Agent {agent.id}")
                ax.scatter(path_coords[0, 1], path_coords[0, 0], color=color, marker='s', s=100, label=f"Start {agent.id}")
                ax.scatter(path_coords[-1, 1], path_coords[-1, 0], color=color, marker='X', s=100, label=f"Goal {agent.id}")
    ...
    def save_gif(self, filename=None, duration=500):
        if filename is None:
            filename = f"{self.env_name}_simulation_paths.gif"
